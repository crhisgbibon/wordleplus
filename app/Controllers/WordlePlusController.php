...
class WordlePlusController extends BaseController
{
  ...
  public function index()
  {
    return view('templates/header')
          ...
          .view('wordleplus/guess')
          .view('wordleplus/list')
  ...
  public function SubmitGuess()
  {
    ...
    for($g = 0; $g < $gCount; $g++)
    {
      for($i = 0; $i < 5; $i++)
      {
        ...
        {
          if($guessToArray[$i] != $answerToArray[$i]) $usedAllLetters = false;
        }
        else if($guessResultArray[$g][$i] == 1)
        {
          $searchForLetter = array_search($guessList[$g][$i], $guessToArray);
    
          if($searchForLetter === false) $usedAllLetters = false;
        }
      }
    }

    if(!$usedAllLetters) return json_encode("NoGood");
  ...
  public function StartDaily()
  {
    ...
    return (string)$this->session->get('dailyWord');
  }
}